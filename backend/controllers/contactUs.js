...
export const contactUs = async (req, res) => {
    try {
        ...
        if (!/^[a-zA-Z0-9._%+-]+@(gmail|hotmail|yahoo|proton|outlook|icloud|test)\.(com|co|me|es|org|edu.co)$/.test(email)) {
            return res.status(400).json({ message: "Escribe un correo electrónico válido (Gmail)." });
        }

        // Buscar usuario por email
        const user = await Users.findOne({ email });
        if (!user) {
            return res.status(400).json({ message: "No se encontró el usuario con ese correo." });
        }
        ...
        const emailOption = {
            ...
            to: process.env.ADMIN_EMAIL || "administrador.dec@gmail.com",
            subject: `Mensaje de ${name} - DEC`,
            html: `
                <div style="font-family: Arial; padding: 20px;">
                    <h2>Nuevo mensaje de contacto</h2>
                    <p><strong>Usuario:</strong> ${name} (${email})</p>
