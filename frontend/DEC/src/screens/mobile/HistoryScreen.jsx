...
const HistoryCard = ({ item }) => {
  // item puede venir de SQLite (camelCase) o de MongoDB (snake_case)
  ...
  return (
    <TouchableOpacity style={[styles.cardContainer, { borderColor: mainColor }]}>
      ...
      <View style={styles.detailsContainer}>
        <Text style={styles.dateText}>{formatSimpleDate(createdAt)}</Text>
...
export default function HistoryScreen() {
  ...
  const loadData = async (refresh = false) => {
    ...
    if (connected && userToken) {
      // Si hay internet, traemos datos frescos del servidor y guardamos localmente
      try {
      ...
      // Sin internet: mostrar solo lo que hay en SQLite
      const localData = getAllRemoteDetections();
      setHistory(localData);
    }
    setLoading(false);
...
// Estilos (igual que antes, se omiten por brevedad)
const styles = StyleSheet.create({
  mainContainer: { flex: 1, backgroundColor: '#F5F7FA' },
  mainHeader: { fontSize: 22, fontWeight: 'bold', color: '#1A1C1E', textAlign: 'center', marginVertical: 20 },
  listContent: { paddingHorizontal: 16, paddingBottom: 20 },
  cardContainer: { flexDirection: 'row', backgroundColor: '#FFF', borderRadius: 15, marginBottom: 16, borderWidth: 2, padding: 15, alignItems: 'center', elevation: 4 },
  imageContainer: { width: 100, height: 100, borderRadius: 10, overflow: 'hidden', backgroundColor: '#E1E4E8' },
  image: { width: '100%', height: '100%' },
  detailsContainer: { flex: 1, paddingLeft: 15, justifyContent: 'center' },
  dateText: { fontSize: 12, color: '#7F8C8D', fontWeight: '600', textTransform: 'capitalize', marginBottom: 2 },
  diseaseName: { fontSize: 20, fontWeight: 'bold', marginBottom: 5 },
  confidenceBadge: { backgroundColor: '#F0F4F8', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8, alignSelf: 'flex-start' },
  confidenceText: { fontSize: 12, color: '#2C3E50', fontWeight: 'bold' },
