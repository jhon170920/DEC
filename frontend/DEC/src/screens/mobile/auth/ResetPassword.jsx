...
export default function ResetPassword() {
  ...
  const { sp, hPad, logoRingS, logoImgS, headlineS, sublineS, fieldH, btnH, iconS } =
    useResponsiveLayout();
  ...
  const inputs = useRef([]);
  const [focusedIndex, setFocusedIndex] = useState(null); // estado para el input enfocado
  ...
  useEffect(() => {
    if (!email) {
      Alert.alert(
        "Error",
        "No se encontró el correo. Por favor, solicita nuevamente la recuperación."
      );
  ...
  // Pegar código completo desde el portapapeles (solo en el primer input)
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          ...
          >
            ...
            <Text
              style={[
                styles.title,
                { fontSize: headlineS, textAlign: "center" },
              ]}
            >
            ...
            <View style={styles.card}>
              {/* Código 6 dígitos */}
              <Text style={[styles.label, { fontSize: sublineS }]}>
              ...
              <View style={styles.codeRow}>
                {codigo.map((digit, index) => (
                  <TextInput
                    ...
                    textAlign="center"
                    editable={!loading}
                    onPaste={index === 0 ? handlePaste : undefined} // solo primer input permite pegar
                  />
                ))}
              </View>
